...
2. **Logical level** (implemented in `logical_level.py`):
  ...
  - The Qiskit transpiler is used here to generate circuits in a suitable fault-tolerant basis set (such as `Clifford+T/Tdg` or `Clifford+Rz`). Based on this, the relative counts of Cliffords vs Non-Cliffords vs Non-Unitary operations in the circuit can be tabulated and visualized. Additionally, individual logical gate breakdowns can be analyzed as well (for each of these gate classes).
